...
@Component({
  selector: 'app-executor-queued-redshift-wet',
  templateUrl: './executor-queued-redshift-wet.component.html',
  styleUrls: ['./executor-queued-redshift-wet.component.css']
})
export class ExecutorQueuedRedshiftWetComponent {
  public chart: any;
  ...
  createChart(){
    this.chart = new Chart("ExecutorQueuedRedshiftWetChart", {
      type: 'line',
      ...
      data: {
        ...
        datasets: [{
          ...
          label: "AWS EC2 #1",
          borderColor: "#34c0eb",
          backgroundColor: "rgb(0, 252, 210, 0.5)",
          fill: true,
          borderWidth: 1.5,
          pointRadius: 0
        }, {
          data: [],
          label: "AWS EC2 #2",
          borderColor: "rgb(249 1 125)",
          backgroundColor: "rgb(235, 52, 186, 0.5)",
          fill: true,
          borderWidth: 1.5,
          pointRadius: 0,
        }],
      ...
      options: {
         ...
         plugins: {
          title: {
              display: true,
              text: 'Executor Queued Redshift Wet Connections',
              font: {
                size: 16
              }
          }
  ...
  private updateChartWithLastMeasurementValues(): void {
    this.chart.data.datasets[0].data.push(this.executorService.getLastMeasurementForInstance(1).queuedRedshiftWet);
    this.chart.data.datasets[1].data.push(this.executorService.getLastMeasurementForInstance(2).queuedRedshiftWet);
  ...
  private loadPastMeasurements(): void {
    ...
    .subscribe(([dbPoolDataInstance1, dbPoolDataInstance2]) => {
      this.measurementInstance2 = dbPoolDataInstance2.map( m => m.queuedRedshiftWet);
      this.measurementInstance1 = dbPoolDataInstance1.map( m => m.queuedRedshiftWet);

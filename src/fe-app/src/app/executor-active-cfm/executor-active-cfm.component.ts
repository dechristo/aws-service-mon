...
@Component({
  selector: 'app-executor-active-cfm',
  templateUrl: './executor-active-cfm.component.html',
  styleUrls: ['./executor-active-cfm.component.css']
})
export class ExecutorActiveCfmComponent {
  public chart: any;
  ...
  createChart(){
    this.chart = new Chart("ExecutorActiveCfmChart", {
      type: 'line',
      ...
      options: {
        ...
        plugins: {
          title: {
            display: true,
            text: 'Executor Active Cfm Connections',
            font: {
              size: 16
            }
          }
  ...
  private updateChartWithLastMeasurementValues(): void {
    this.chart.data.datasets[0].data.push(this.executorService.getLastMeasurementForInstance(1).activeCfm);
    this.chart.data.datasets[1].data.push(this.executorService.getLastMeasurementForInstance(2).activeCfm);
  ...
  private loadPastMeasurements(): void {
    forkJoin([this.executorService.loadMeasurementHistory(1), this.executorService.loadMeasurementHistory(2)])
      .subscribe(([dbPoolDataInstance1, dbPoolDataInstance2]) => {
        this.measurementInstance2 = dbPoolDataInstance2.map( m => m.activeCfm);
        this.measurementInstance1 = dbPoolDataInstance1.map( m => m.activeCfm);

...
@Component({
  selector: 'app-redshift-inline-db-pool-chart',
  templateUrl: './redshift-inline-db-pool-chart.component.html',
  styleUrls: ['./redshift-inline-db-pool-chart.component.css']
})

export class RedshiftInlineDbPoolChartComponent {
  public chart: any;
  ...
  createChart(){
    this.chart = new Chart("RedshiftInlinePoolPerInstanceChart", {
      type: 'line',
      ...
      data: {
        ...
        datasets: [{
          data: [],
          label: "RS Inline Pool EC2 #1",
          borderColor: "rgb(255,89,0)",
          backgroundColor: 'rgba(254,254,19,0.74)',
          fill: true,
          borderWidth: 1.5,
          pointRadius: 0
        }, {
          data: [],
          label: "RS Inline Pool EC2 #2",
          borderColor: "rgb(7,62,189)",
          backgroundColor: 'rgba(0,225,255,0.68)',
          fill: true,
          borderWidth: 1.5,
          pointRadius: 0
        }],
      ...
      options: {
         ...
         plugins: {
          title: {
            display: true,
            text: 'Redshift Inline Pool active connections',
            font: {
              size: 16
            }
          },
  ...
  private updateChartWithLastMeasurementValues(): void {
    this.chart.data.datasets[0].data.push(this.databasePoolService.getLastMeasurementForInstance(1).redshiftInlineConnectionPool);
    this.chart.data.datasets[1].data.push(this.databasePoolService.getLastMeasurementForInstance(2).redshiftInlineConnectionPool);
  ...
  private loadPastMeasurements(): void {
    forkJoin([this.databasePoolService.loadMeasurementHistory(1), this.databasePoolService.loadMeasurementHistory(2)])
      .subscribe(([dbPoolDataInstance1, dbPoolDataInstance2]) => {
        this.measurementInstance2 = dbPoolDataInstance2.map( m => m.redshiftInlineConnectionPool);
        this.measurementInstance1 = dbPoolDataInstance1.map( m => m.redshiftInlineConnectionPool);

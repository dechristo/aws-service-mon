...
@Component({
  selector: 'app-executor-queued-redshift-sort',
  templateUrl: './executor-queued-redshift-sort.component.html',
  styleUrls: ['./executor-queued-redshift-sort.component.css']
})
export class ExecutorQueuedRedshiftSortComponent {
  public chart: any;
  ...
  createChart(){
    this.chart = new Chart("ExecutorQueuedRedshiftSortChart", {
      type: 'line',
      ...
      options: {
        ...
        plugins: {
          title: {
            display: true,
            text: 'Executor Queued Redshift Sort Connections',
            font: {
              size: 16
            }
          }
  ...
  private updateChartWithLastMeasurementValues(): void {
    this.chart.data.datasets[0].data.push(this.executorService.getLastMeasurementForInstance(1).queuedRedshiftSort);
    this.chart.data.datasets[1].data.push(this.executorService.getLastMeasurementForInstance(2).queuedRedshiftSort);
  ...
  private loadPastMeasurements(): void {
    forkJoin([this.executorService.loadMeasurementHistory(1), this.executorService.loadMeasurementHistory(2)])
      .subscribe(([dbPoolDataInstance1, dbPoolDataInstance2]) => {
        this.measurementInstance2 = dbPoolDataInstance2.map( m => m.queuedRedshiftSort);
        this.measurementInstance1 = dbPoolDataInstance1.map( m => m.queuedRedshiftSort);
